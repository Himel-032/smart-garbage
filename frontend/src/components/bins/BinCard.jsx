...
import { Trash2, MapPin, User, Droplets, Gauge } from "lucide-react";
...
const BinCard = ({ bin }) => {
  ...
  return (
    <div className={`rounded-xl shadow-md border-2 ${config.borderColor} ${config.bgColor} overflow-hidden transition-all hover:shadow-lg`}>
      ...
      <div className="p-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className={`text-xs font-semibold px-2 py-1 rounded-full ${config.labelBg}`}>
              {config.label}
            </span>
          </div>
        </div>
        <div className="text-right">
          <p className={`text-2xl font-bold ${config.binColor}`}>{fillPercentage}%</p>
      ...
      <div className="px-4 pb-4 space-y-2">
        ...
        <div className="flex items-center gap-2 text-gray-600">
          ...
          <span className="text-sm">
            {bin.driver_name || (bin.driver_id ? `Driver #${bin.driver_id}` : "Unassigned")}
          </span>
        </div>
        ...
        <div className="flex items-center justify-between text-sm">
          <div className="flex items-center gap-2 text-gray-600">
            ...
            <span>{bin.current_level}L / {bin.capacity}L</span>
          </div>
          <div className="flex items-center gap-2 text-gray-600">
